...
└──────────────────────┬───────────────────────────────┘
                       │ VPN / TLS → 10.1.10.51 (Bastion 외부 IP)
┌──────────────────────▼───────────────────────────────┐
│  Bastion (이중 NIC — 유일한 외부 접점)                 │
│  ens33: 10.1.10.51 (외부)  ens35: 192.168.0.1 (내부) │
│  DNS · HAProxy · Mirror Registry · NAT Gateway        │
└──────────────────────┬───────────────────────────────┘
                       │ 192.168.0.0/24 (내부 전용망)
┌──────────────────────▼───────────────────────────────┐
...
└──────────────────────┬───────────────────────────────┘
                       │ InfiniBand / 10GbE (192.168.0.x)
┌──────────────────────▼───────────────────────────────┐
│         H100 × 1 AI 플랫폼 (192.168.0.16, 5월)        │
│                                                       │
...
## 2-1. 네트워크 설계 (이중 NIC 분리)

### 목적: 외부 IP 절약 — Bastion 1개만 외부망 노출

| 노드 | 외부 IP (10.x) | 내부 IP (192.168.0.x) | NIC |
|------|---------------|----------------------|-----|
| Bastion | **10.1.10.51** | **192.168.0.1** | ens33 (외부) + ens35 (내부) |
| Bootstrap | — | **192.168.0.10** | ens35 (내부 전용) |
| Master 1 | — | **192.168.0.11** | ens35 (내부 전용) |
| Master 2 | — | **192.168.0.12** | ens35 (내부 전용) |
